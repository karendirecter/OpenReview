...
```bash
git add pyproject.toml app/__init__.py app/main.py app/config.py .env.example tests/unit/test_config.py
...
Run: `uv run pytest tests/unit/test_orchestrator.py::test_review_task_binds_review_commit_sha -v`
Expected: FAIL with missing `ReviewTask` or `IssueHit`

- [ ] **Step 3: Write minimal domain models**
...
```bash
git add app/review/models.py tests/unit/test_orchestrator.py
...
- [ ] **Step 1: Write the failing diff parser test**
...
```bash
git add app/review/diff_parser.py tests/unit/test_diff_parser.py tests/integration/fixtures/sample_pr_diff.patch
