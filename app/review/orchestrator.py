...
def review_pull_request(task: ReviewTask, llm_client: LLMClient | None = None) -> ReviewResult:
    ...
    findings = run_stage_two(task, candidate_hits, llm_client) if candidate_hits else []
    if not findings:
        findings = [issue_hit_to_finding(task, hit) for hit in candidate_hits]

    summary, risk = summarize_findings(findings)
    return ReviewResult(
        review_commit_sha=task.review_commit_sha,
        summary=summary,
        overall_risk=risk,
        findings=findings,
        stats={"stage1_candidates": len(candidate_hits), "stage2_findings": len(findings)},
...
def run_stage_two(task: ReviewTask, candidate_hits: list[IssueHit], llm_client: LLMClient | None) -> list[ReviewFinding]:
    ...
    for hit in candidate_hits[:5]:
        ...
        findings.extend(validate_llm_payload(payload, {hit.file_path}, task.review_commit_sha))
...
def summarize_findings(findings: list[ReviewFinding]) -> tuple[str, str]:
    if not findings:
