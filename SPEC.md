...
- **Open Design skill：`dashboard`**：本项目的前端不是营销页，而是偏运维/审查后台的监控台，因此优先选择 `dashboard` skill，而不是 `landing`、`docs` 或 `mobile-app`。它更适合承载“历史列表、详情面板、指标卡片、回放操作”这类信息密度较高的后台交互。
- **Open Design 设计系统：Dashboard 风格控制台**：前端视觉语言选定为 dashboard 风格，而不是品牌营销型或内容型设计系统。理由是本项目的目标用户是开发者与仓库维护者，他们更关注状态可视化、信息分区、回放入口与问题定位效率；dashboard 风格在“可扫描性、模块化、局部高亮、后台控制感”上与该使用场景更匹配。
- **选择理由总结**：相较于通用极简页面，dashboard 风格更能降低“AI 生成界面千篇一律”的问题，并且与本项目的本地监控器定位一致，适合作为 PR review 历史、Agent trace 和 replay 的统一操作界面。
...
- **Render Web Service**：作为 MVP 的优先云部署平台。理由是它对 Docker 化 Web 服务支持直接，适合课程项目快速交付公开可访问地址，也便于将 GitHub webhook、FastAPI 服务与前端静态资源统一部署在单个容器中。
- **单容器部署策略**：后端 FastAPI 同时提供 `/api/*` 接口并托管 `frontend/dist` 静态资源，减少前后端分离带来的跨域、代理和多服务编排复杂度，更适合课程项目与本地调试环境保持一致。
- **Docker 作为部署中间层**：无论本地使用 `docker compose`，还是云端使用 Render，均以同一个 `Dockerfile` 作为交付边界，降低“本地能跑、线上失败”的环境漂移风险。
...
1. `/review` 触发后必须先创建 `ReviewRun`，再进入后台队列，而不是在 webhook 请求线程中同步跑完整条审查。
2. 系统必须支持“不同 PR 并发、同一 PR 串行”的执行模型；同一 PR 的重复触发不得产生重叠运行中的有效任务。
3. 任务状态至少应覆盖 `queued`、`running`、`completed`、`failed`、`cancelled`、`stale`，并在监控器中可见。
4. 在评论发布前，系统必须重新检查当前 PR head 是否仍等于 `review_commit_sha`；若不一致，任务必须标记为 `stale` 并停止评论发布。
5. 容器重启后，数据库中残留的 `queued` / `running` 任务必须可恢复；恢复过程中同一 PR 只保留最新任务，旧任务标记为 `cancelled`。
6. Review 历史、agent trace 与队列恢复所需状态必须持久化；容器重建不应导致监控器历史与待执行任务整体丢失。
...
10. **绑定多个仓库时的并发问题**
   - 主要体现在跨PR地对数据库数据并发写的竞争

### 11.2 缓解思路

- 使用 `review_commit_sha`，PR串行锁，任务过期机制控制漂移
...
- 通过 持久化PR作业队列，协调数据库写并发，支持事务回滚

### 11.3 当前未决问题

- 已决：Python 深度规则首版采用 **AST 启发式分析 + Semgrep 补充分析** 的组合策略
- 已决：AST 分析器负责快速检测 None access、async blocking I/O 等常见模式
- 已决：Semgrep 用于补充资源泄漏及更复杂的模式匹配规则
- 已决：本地隧道联调方案（如具体工具选择）尚未在本规约中锁定，可在实现计划中补充
